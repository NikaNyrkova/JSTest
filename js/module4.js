...
// Решение задачи в две строчки. Но нужно очень хорошо разобраться с методом map(), изучить документацию и можно посмотреть видосики на ютюбе.
// Если коротко:
// const newArray = arr.map(function callback(currentValue[, index[, array]]) {return ...}
// перебрав масив arr, параметром метода map() есть ф-я callback которая принимает три аргумента: currentValue (поточный елемент массива arr), index( индекс поточного елемента arr), array( сам массив arr) и возвращает новый массив, каждый елемент которого результат ф-и callback. В нашем случаи возврат ф-и callback это вызов ф-и composeMessage с методом .call (для вызова функции в контексте одного объекта-заказа). А дальше внимательно изучить условие задачи, там расписан ход решения.
